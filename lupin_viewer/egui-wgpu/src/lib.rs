...
impl RenderState {
    ...
    pub async fn create(
        ...
        let available_adapters = {
            ...
            wait_for(instance.enumerate_adapters(backends))
        };
    ...
    F: std::future::Future,
{
    ...
    loop
    {
        match t.poll(&mut context)
        {
            ...
            std::task::Poll::Pending => panic!("This executor does not support futures that are not ready"),
        }
    }
}
...
unsafe fn clone(_: WakerData) -> std::task::RawWaker { my_raw_waker() }
unsafe fn wake(_: WakerData) {}
unsafe fn wake_by_ref(_: WakerData) {}
unsafe fn drop(_: WakerData) {}

static MY_VTABLE: std::task::RawWakerVTable = std::task::RawWakerVTable::new(clone, wake, wake_by_ref, drop);

fn my_raw_waker() -> std::task::RawWaker { return std::task::RawWaker::new(std::ptr::null(), &MY_VTABLE) }

fn my_waker() -> std::task::Waker { unsafe { return std::task::Waker::from_raw(my_raw_waker()) } }
